...
async function getModel(service, yamlSchema = js_yaml_1.DEFAULT_SAFE_SCHEMA) {
    ...
    return {
        filename,
        model: codegen_1.deserialize(await service.ReadFile(filename), filename, yamlSchema)
    };
}
class Session {
    ...
    /* @internal */ async init(project, schema = js_yaml_1.DEFAULT_SAFE_SCHEMA) {
        const m = await getModel(this.service, schema);
        this.model = m.model;
    ...
    async getValue(key, defaultValue) {
        ...
        if (value === null || value === undefined) {
            try {
                value = codegen_1.safeEval(key, this.context);
            }
            catch (_a) {
                value = null;
            }
...
async function startSession(service, project, schema = js_yaml_1.DEFAULT_SAFE_SCHEMA) {
    return await new Session(service).init(project, schema);
}
